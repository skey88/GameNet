...
public class sEvent_NetMessageData
{
...
{
    private Dictionary<MessageType, Callback_NetMessage_Handle> _netMessage_EventList = new Dictionary<MessageType, Callback_NetMessage_Handle>();
    ...
    //添加网络事件观察者
    public void addObsever(MessageType _protocalType, Callback_NetMessage_Handle _callback)
    {
    ...
    //删除网络事件观察者
    public void removeObserver(MessageType _protocalType, Callback_NetMessage_Handle _callback)
    {
        if (_netMessage_EventList.ContainsKey(_protocalType))
        {
            _netMessage_EventList[_protocalType] -= _callback;
    ...
    { 
        ...
        while (_netMessageDataQueue.Count > 0)
        {
            lock (_netMessageDataQueue)
            {
                ...
                //    Game.Get().SendNotification(RGGameGlobalEventType.NET_RECEIVES_NETWORK_NEWS, tmpNetMessageData._eventType);
